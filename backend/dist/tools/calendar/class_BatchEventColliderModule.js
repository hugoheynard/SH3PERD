...
export class BatchEventColliderModule {
    ...
    sortList_DenseActivityPeriodFirst(input) {
        ...
        const split = new Date(sortEventsArrayPerAscendingTime(data)[0].date);
    ...
    calculateCollisions() {
        ...
        for (let i = 0; i < events.length; i++) {
            for (let j = i + 1; j < events.length; j++) {
                ...
                this.addColliderToListIfCollide(new EventCollider({
                    referenceEvent: referenceEvent,
                    comparedEvent: comparedEvents
                }));
            }
...
//# sourceMappingURL=class_BatchEventColliderModule.js.map