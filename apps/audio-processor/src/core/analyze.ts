...
async function decodeWithFFmpeg(inputBuffer: Buffer): Promise<DecodedAudio> {
  ...
  const probeInfo = await new Promise<{ sampleRate: number; channels: number }>((resolve, reject) => {
    ...
    proc.on('close', code => {
      if (code !== 0) { reject(new Error(`ffprobe exit ${code}`)); return; }
      try {
  ...
  // Deinterleave
  const float32 = new Float32Array(rawBuffer.buffer, rawBuffer.byteOffset, totalSamples);
...
function analyzeWithEssentia(monoSamples: Float32Array): EssentiaResult {
  ...
  } catch { /* BPM detection can fail on very short or silent audio */ }
  ...
  } catch { /* Key detection can fail on percussive/atonal audio */ }
...
function computeAnalysis(
  ...
  const SNRdB = (isFinite(integratedLUFS) && isFinite(noiseFloorLUFS))
    ? integratedLUFS - noiseFloorLUFS : 0;
 ...
 */
export async function analyzeAudioFile(fileBuffer: Buffer): Promise<TAudioAnalysisSnapshot> {
  const decoded = await decodeWithFFmpeg(fileBuffer);
  const metrics = computeAnalysis(decoded.channels, decoded.sampleRate, decoded.duration);
