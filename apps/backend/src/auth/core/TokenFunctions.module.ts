...
import { VERIFY_AUTH_TOKEN_FN, VERIFY_REFRESH_TOKEN_FN } from '../../appBootstrap/nestTokens.js';
  ...
  providers: [
    {
      ...
      useFactory:
        (jwtService: IAbstractJWTService): TVerifyAuthTokenFn =>
        (x) =>
          jwtService.verifyAuthToken(x),
      inject: [JWT_SERVICE],
    },
    {
      provide: VERIFY_REFRESH_TOKEN_FN,
      useFactory:
        (refreshTokenService: IAbstractRefreshTokenService): TVerifyRefreshTokenFn =>
        (x) =>
          refreshTokenService.verifyRefreshToken(x),
