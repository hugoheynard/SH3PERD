...
describe('RefreshTokenManager', () => {
  ...
  const generatorFnMock = jest.fn(async () => mockToken) as jest.MockedFunction<
  ...
  const deleteRefreshTokenFnMock = jest.fn(async (input: { refreshToken: TRefreshToken }) => ({
    revokedToken: input.refreshToken,
  }));

  //TODO FOR SERVICE
  const findRefreshTokenMock = jest.fn(
    async (input: { refreshToken: TRefreshToken }): Promise<TRefreshTokenDomainModel | null> => ({
  ...
  it('should throw if token generation fails', async () => {
    ...
    await expect(sut.generateRefreshToken({ user_id: mockUserId })).rejects.toThrow(
  ...
  it('should throw if saveRefreshToken throws', async () => {
    (saveRefreshTokenFnMock as jest.MockedFunction<TSaveRefreshTokenFn>).mockImplementationOnce(
      async () => {
    ...
    await expect(sut.generateRefreshToken({ user_id: mockUserId })).rejects.toThrow(
      `Unable to save refresh token for user ${mockUserId}: DB failure`,
    );
  });

  it('should throw if revokeRefreshToken throws', async () => {
    (deleteRefreshTokenFnMock as jest.MockedFunction<TDeleteRefreshTokenFn>).mockImplementationOnce(
      async () => {
        throw new Error('Revoke error');
      },
    );

    await expect(sut.revokeRefreshToken({ refreshToken: mockToken })).rejects.toThrow(
      'Unable to revoke refresh token: Revoke error',
    );
  });
});
