...
describe('assertRequiredKeys', () => {
  it('should not throw if all required keys are present and are functions', () => {
    ...
    expect(() => assertRequiredKeys<typeof input>(input, ['a', 'b'], 'TestDeps')).not.toThrow();
  });
  ...
  it('should throw if input is not an object', () => {
    expect(() => assertRequiredKeys<any>(null, ['a'], 'NullInput')).toThrow(
      /Expected NullInput to be a non-null object/,
    );

    expect(() => assertRequiredKeys<any>('string', ['a'], 'StringInput')).toThrow(
      /Expected StringInput to be a non-null object/,
    );
  });
});
