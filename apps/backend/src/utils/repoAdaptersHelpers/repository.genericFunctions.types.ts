...
export type TGenericRepoFindOneFn<TRecord> = (
  filter: Partial<TRecord> | Filter<TRecord>,
 ...
 */
export type TGenericRepoUpdateOneFn<TRecord> = (input: {
  filter: Partial<TRecord> | Filter<TRecord>;
  update: Partial<TRecord> | UpdateFilter<TRecord>;
}) => Promise<TRecord | null>;

export type TSaveMongoFn<TRecord> = (
  docOrDocs: OptionalUnlessRequiredId<TRecord> | OptionalUnlessRequiredId<TRecord>[],
  session?: ClientSession,
) => Promise<boolean>;
export type TFindOneMongoFn<TRecord> = (input: {
  filter: Filter<TRecord>;
}) => Promise<TRecord | null>;
export type TFindManyMongoFn<TRecord> = (input: {
  filter: Filter<TRecord>;
}) => Promise<TRecord[] | null>;
export type TUpdateOneMongoFn<TRecord> = (input: {
  filter: Filter<TRecord>;
