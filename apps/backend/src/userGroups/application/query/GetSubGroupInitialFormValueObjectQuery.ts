...
import type {
  ...
  TContractId,
  TFormOption,
  TSubgroupInitialFormValuesObject,
...
export type TGetSubGroupInitialFormValueObjectUseCase = (input: {
  context: TUseCaseContext<'scoped'>;
  parentUserGroupId: TUserGroupId;
}) => Promise<TSubgroupInitialFormValuesObject>;
...
@Injectable()
export class GetSubGroupInitialFormValueObjectHandler implements IQueryHandler<
  GetSubGroupInitialFormValueObjectQuery,
  TSubgroupInitialFormValuesObject
> {
  constructor(
  ...
  async execute(
    input: GetSubGroupInitialFormValueObjectQuery,
  ): Promise<TSubgroupInitialFormValuesObject> {
    ...
    const contractsWithProfile = await this.ctrReadRepo.getContractWithUserProfile(
      parentUg.uniqueIds,
    );
    const aggregate = UserGroupAggregate.create(
      parentUg,
      contractsWithProfile.map((c) => new ContractEntity(c.contract as any)),
    );
  ...
  //--- Helpers ---//
  buildMembersFormOptions(
    contractsWithProfile: any[],
    allowedMembers: ContractEntity[],
  ): TFormOption<TContractId>[] {
    const allowedIds = new Set(allowedMembers.map((m) => m.id));

    const options: TFormOption<TContractId>[] = [];
  ...
  transformToFormOptions<T>(array: { label?: string; value: T }[]): TFormOption<T>[] {
