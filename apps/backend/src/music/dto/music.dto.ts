...
export class AudioAnalysisPayload {
  @ApiProperty({ example: -10.34, description: 'Integrated loudness (ITU-R BS.1770)' }) integratedLUFS!: number;
  ...
  @ApiProperty({ example: 0, description: 'Ratio of clipped samples (0–1)' }) clippingRatio!: number;
  ...
  @ApiProperty({ example: 'major', enum: KEY_SCALES, description: 'Major or minor' }) keyScale!: string;
  @ApiProperty({ example: 0.831, description: 'Key detection confidence (0–1)' }) keyStrength!: number;
  ...
  @ApiProperty({ example: 3, enum: RATINGS, description: 'Audio quality rating 1–4' }) quality!: number;
}
...
export class VersionTrackPayload {
  ...
  @ApiProperty({ required: false, type: () => AudioAnalysisPayload }) analysisResult?: AudioAnalysisPayload;
  @ApiProperty({ example: true, description: 'Favorite track is used for version-level metrics' }) favorite!: boolean;
  @ApiProperty({ required: false, example: 'track_xyz-456', description: 'Source track this was derived from' }) parentTrackId?: string;
  @ApiProperty({ required: false, enum: PROCESSING_TYPES, description: 'How this track was processed from its parent' }) processingType?: string;
  @ApiProperty({ required: false, example: 'tracks/user_abc/ver_123/track_456/file.mp3' }) s3Key?: string;
}
...
export class MusicVersionPayload {
  ...
  @ApiProperty({ example: 'musicRef_xyz-456', description: 'Music reference ID (prefixed)' }) musicReference_id!: string;
  ...
  @ApiProperty({ nullable: true, example: 120, description: 'User-defined BPM (null if not set)' }) bpm!: number | null;
  @ApiProperty({ nullable: true, example: null, description: 'User-defined pitch (null if not set)' }) pitch!: number | null;
  ...
  @ApiProperty({ required: false, example: 'musicVer_parent-123', description: 'Source version ID (for derived versions)' }) parentVersionId?: string;
  @ApiProperty({ required: false, enum: DERIVATION_TYPES, description: 'How this version was derived from its parent' }) derivationType?: string;
}
...
export class MusicReferencePayload {
  @ApiProperty({ example: 'musicRef_abc-123', description: 'Music reference ID (prefixed)' }) id!: string;
...
export class RepertoireEntryPayload {
  @ApiProperty({ example: 'repEntry_abc-123', description: 'Repertoire entry ID (prefixed)' }) id!: string;
  @ApiProperty({ example: 'musicRef_xyz-456', description: 'Music reference ID (prefixed)' }) musicReference_id!: string;
  @ApiProperty({ example: 'user_abc-123', description: 'Owner user ID (prefixed)' }) owner_id!: string;
}
...
export class VersionViewPayload {
  ...
  @ApiProperty({ enum: RATINGS, example: 2, description: 'Effort rating 1–4' }) effort!: number;
  @ApiProperty({ type: () => [VersionTrackPayload] }) tracks!: VersionTrackPayload[];
  @ApiProperty({ required: false, example: 'musicVer_parent-123', description: 'Source version ID (prefixed)' }) parentVersionId?: string;
...
export class UserMusicLibraryViewModelPayload {
  @ApiProperty({ type: () => [RepertoireEntryViewModelPayload] }) entries!: RepertoireEntryViewModelPayload[];
}
...
export class TrackDownloadUrlPayload {
  @ApiProperty({ example: 'https://r2.cloudflarestorage.com/...', description: 'Presigned download URL (expires after 1h)' }) url!: string;
}
...
export class MasteringTargetSpecsPayload {
  @ApiProperty({ example: -14, description: 'Target integrated loudness in LUFS' }) targetLUFS!: number;
