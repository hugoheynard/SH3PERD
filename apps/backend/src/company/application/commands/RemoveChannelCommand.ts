...
/** Removes a channel from the company. Requires `company:settings:write`. */
@CommandHandler(RemoveChannelCommand)
export class RemoveChannelHandler implements ICommandHandler<RemoveChannelCommand, TCompanyRecord> {
  ...
  async execute(cmd: RemoveChannelCommand): Promise<TCompanyRecord> {
    ...
    entity.removeChannel(cmd.channelId);
    ...
    if (!updated) throw new BusinessError('Failed to remove channel', 'COMPANY_UPDATE_FAILED', 500);
    return updated;
  }
}
