...
export class UserLookupService extends BaseHttpService {
  searchUserByEmail(
    email: string,
  ): Observable<TApiResponse<TUserSearchResult | null>> {
    return this.http.get<TApiResponse<TUserSearchResult | null>>(
      `${this.UrlBuilder.apiProtectedRoute('user').route('search').build()}?email=${encodeURIComponent(email)}`,
    );
  }
  ...
  inviteUser(dto: {
    ...
    first_name: string;
    last_name: string;
  }): Observable<TApiResponse<TUserSearchResult>> {
    return this.http.post<TApiResponse<TUserSearchResult>>(
      this.UrlBuilder.apiProtectedRoute('user').route('invite').build(),
      dto,
    );
  }
}
