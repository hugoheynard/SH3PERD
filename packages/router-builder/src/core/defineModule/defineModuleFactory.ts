 ...
 * ```
*/
export type TModuleInput<TDeps extends RouteFactoryContext> = {
    path: `/${string}`;
    inject?: TDeps;
    ...
    return function defineModule<TDeps extends RouteFactoryContext>(
        ...
        return {
            ...
            factory: async (context: RouteFactoryContext) => { //
                const typedContext = (input.inject ?? {}) as TDeps;

                const rawRoutes = input.routes ? input.routes(typedContext) : {};
