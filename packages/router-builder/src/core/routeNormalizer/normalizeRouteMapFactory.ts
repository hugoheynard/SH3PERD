    ...
    return async function normalizeRouteMap<TDeps extends Record<string, unknown>>(
        ...
        for (const key in rawRoutes) {
            ...
            if (deps.isHandlerFn(route)) {
                final[key as keyof RouteMap] = [route];
                continue;
            }
            ...
            if (deps.isObjectRouteFn(route)) {
                final[key as keyof RouteMap] = await deps.buildFromObjectRouteFn({
                    key: key,
                    routeObject: route,
                    context: context
                });
            ...
            final[key as keyof RouteMap] = [deps.defaultHandlerFn(method, path)];
        }

        deps.validateRouteMapFn(final);
        return final;
    };
}
